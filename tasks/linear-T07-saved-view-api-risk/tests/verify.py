...
TASK_ID = 'linear-T07'
CHECKS: list[tuple[str, str]] = [('view_exists_for_alex', "SELECT EXISTS (SELECT 1 FROM saved_views v JOIN users u ON u.id = v.owner_id WHERE v.name = 'Mine: High API Risk' AND u.username = 'alex.rivera')"), ('view_is_personal', "SELECT EXISTS (SELECT 1 FROM saved_views WHERE name = 'Mine: High API Risk' AND scope = 'personal')"), ('view_filters_platform', "SELECT EXISTS (SELECT 1 FROM saved_views WHERE name = 'Mine: High API Risk' AND filters_json::text ILIKE '%PLAT%')"), ('view_filters_api_security', "SELECT EXISTS (SELECT 1 FROM saved_views WHERE name = 'Mine: High API Risk' AND filters_json::text ILIKE '%API%' AND filters_json::text ILIKE '%Security%')"), ('view_display_grouped_by_status', "SELECT EXISTS (SELECT 1 FROM saved_views WHERE name = 'Mine: High API Risk' AND display_json::text ILIKE '%status%')")]
