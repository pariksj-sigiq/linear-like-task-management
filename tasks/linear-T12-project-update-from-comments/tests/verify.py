...
TASK_ID = 'linear-T12'
CHECKS: list[tuple[str, str]] = [('billing_project_at_risk', "SELECT EXISTS (SELECT 1 FROM projects WHERE id = 'prj-billing-polish' AND health = 'at_risk')"), ('update_exists', "SELECT EXISTS (SELECT 1 FROM project_updates WHERE project_id = 'prj-billing-polish' AND health = 'at_risk')"), ('update_mentions_chargeback', "SELECT EXISTS (SELECT 1 FROM project_updates WHERE project_id = 'prj-billing-polish' AND body ILIKE '%chargeback%')"), ('update_mentions_invoice_export', "SELECT EXISTS (SELECT 1 FROM project_updates WHERE project_id = 'prj-billing-polish' AND body ILIKE '%invoice export%')"), ('update_mentions_tax_rounding', "SELECT EXISTS (SELECT 1 FROM project_updates WHERE project_id = 'prj-billing-polish' AND body ILIKE '%tax rounding%')")]
