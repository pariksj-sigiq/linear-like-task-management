...
const primaryNav = [
  { label: "Inbox", path: "/inbox", icon: Inbox, testId: "nav-inbox" },
  { label: "My Issues", path: "/my-issues", icon: UserRoundCheck, testId: "nav-my-issues" },
  ...
  { label: "Roadmap", path: "/roadmap", icon: Map, testId: "nav-roadmap" },
];

const teamNav = [
  { label: "All", segment: "all" },
    ...
    <div className="linear-app app-layout">
      <aside className="sidebar" data-testid="linear-sidebar">
        <div className="sidebar-header">
          ...
          <div className="workspace-name truncate">Linear Clone</div>
          <Button variant="ghost" iconOnly onClick={() => setQuickCreateOpen(true)} aria-label="Create issue" data-testid="sidebar-create-button">
            <Plus size={15} />
          </Button>
        </div>
        ...
        <div className="sidebar-scroll">
          ...
          <section className="sidebar-section" aria-label="Teams">
            ...
            {teams.map((team) => (
              <div key={team.key}>
                <NavLink to={`/team/${team.key.toLowerCase()}/all`} className="nav-item" data-testid={`team-${team.key.toLowerCase()}-nav`}>
      ...
      <main className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <Button onClick={() => setCommandOpen(true)} data-testid="command-palette-button">
              <Search size={14} />
              Search
              <Kbd>⌘K</Kbd>
            </Button>
            <Button variant="primary" onClick={() => setQuickCreateOpen(true)} data-testid="quick-create-button">
              <Plus size={14} />
              New issue
            </Button>
